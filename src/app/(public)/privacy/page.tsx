  ...
  return (
    <PageShell profile={profile} title="Privacy.txt - Notepad">
      <article className="privacy-note">
        ...
        <section><h2>Guestbook and Turnstile</h2><p>Guestbook messages and optional display names are public and stored in Neon. Cloudflare Turnstile checks submissions for spam. Turnstile may process device and network information under Cloudflare&apos;s own policies.</p></section>
        <section><h2>Private anonymous messages</h2><p>The anonymous message form stores only the message in Neon. It does not ask for a name, email address, social account, or location. The signed visitor cookie is converted into a one-way hash for a separate three-message-per-day limit. Cloudflare Turnstile also checks each private submission.</p></section>
